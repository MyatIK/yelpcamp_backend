            ...
            <div className="grid grid-cols-1 lg:grid-cols-5">
                <div className="m-5 p-2 lg:col-span-3 lg:m-10 lg:p-20">
                    ...
                    <form className="mt-10 lg:mt-20">
                        <fieldset>
                            <legend className="text-3xl font-bold mb-5 ">Start exploring camps from all around the world.</legend>
                            <label className="block mt-3 mb-3" for="username">Username</label>
                            <input  className="block bg-stone-100 rounded p-3 w-4/5 lg:w-full" type="text" onChange={(e)=>setName(e.target.value)} value={name} placeholder='johndoe_91' id="username"/>
                            <label className="block mt-3 mb-3" for="password">Password</label>
                            <input type="password" placeholder="Enter Your Password" id="password" onChange={(e)=>setPassword(e.target.value)} value={password} className="block bg-stone-100 rounded p-3 w-4/5 lg:w-full"/>
                        </fieldset>
                    </form>

                       
                        
                    <Link to="/search"><button onClick={signIn} disabled={!isEnabled} className="rounded bg-black text-white w-4/5 p-3 w-full mt-3">Login</button></Link>
